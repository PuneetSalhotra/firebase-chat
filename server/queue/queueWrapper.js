    ...
    this.raiseFormWidgetEvent = function (event, activityId, callback) {
        ...
        producer.send(payloads, function (err, data) {
            if (err) {                
                ...
                util.logError({},`serverError error in producing data Error %j`, {error : err,payload : event.payload });
                // callback(true, err);
            } else {
                //global.logger.write('debug', 'Producer success callback message ' + JSON.stringify(data), JSON.stringify(data), event.payload);
                util.logInfo({},`debug Producer success callback message %j`,{message : JSON.stringify(data),JSON.stringify(data),payload : event.payload});
                // callback(false, 'Producer success callback message');
            }            
        });        
    };    
