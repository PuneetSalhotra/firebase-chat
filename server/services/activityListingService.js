...
function ActivityListingService(db, util) {
    ...
    this.getCoworkers = function (request, callback) {
        ...
        var queryString = util.getQueryString('ds_v1_activity_asset_mapping_select_category_others', paramsArr);
        if (queryString != '') {
            db.executeQuery(1, queryString, request, function (err, data) {
                if (err === false) {
                    formatActivityListing(data, function (err, finalData) {
    ...
    this.searchActivityByType = function (request, callback) {
        ...
        var queryString = util.getQueryString('ds_v1_activity_asset_mapping_select_search_category_title', paramsArr);
        if (queryString != '') {
            db.executeQuery(1, queryString, request, function (err, data) {
                if (err === false) {
                    formatActivityListing(data, function (err, finalData) {
    ...
    this.searchMail = function (request, callback) {
        ...
        var queryString = util.getQueryString('ds_v1_activity_asset_mapping_select_search_category_mail', paramsArr);
        if (queryString != '') {
            db.executeQuery(1, queryString, request, function (err, data) {
                if (err === false) {
    ...
    var formatActivityListing = function (data, callback) {
        ...
        data.forEach(function (rowData, index) {
            var rowDataArr = {
                ...
                "activity_update_count": util.replaceDefaultString(rowData['asset_unread_updates_count']),
                "asset_id": util.replaceDefaultNumber(rowData['asset_id']),
                "workforce_id": util.replaceZero(rowData['workforce_id']),
