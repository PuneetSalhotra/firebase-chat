...
function FormConfigService(objCollection) {
    ...
    this.alterFormActivity = function(request, callback) {
        ...
        request.new_field_value = newData[0].field_value;
        
        activityCommonService.getActivityDetails(request, request.activity_id, (err, data)=>{
            if(err === false) {
                //console.log('DAta : ', data[0].activity_inline_data);
                
                var x = JSON.parse(data[0].activity_inline_data);               
                
                forEachAsync(x, (next, row)=>{                                      
                   if(Number(row.field_id) === Number(newData[0].field_id)) {
                       row.field_value = newData[0].field_value;
                   }
                   next();
                }).then(()=>{
    ...
    function putLatestUpdateSeqId(request){
        return new Promise((resolve, reject)=>{
            ...
            var temp = JSON.parse(request.activity_inline_data);
            var row = temp[0];
            global.logger.write('debug', row, {}, request);      
            global.logger.write('row field_data_type_id from json : ' + row.field_data_type_id, {}, request);
