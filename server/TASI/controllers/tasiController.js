...
function TasiController(objCollection) {
    ...
    app.post('/' + global.config.version + '/tasi/payout/policy/list', async function (req, res) {
        const [err, resData] = await tasiService.listPayoutPolicies(req.body);
        if (!err) {
            res.json(responseWrapper.getResponse({}, resData, 200, req.body));
        } else {
            console.log("/tasi/payout/policy/list | Error: ", err);
            res.json(responseWrapper.getResponse(err, resData, -9999, req.body));
        }
    });
